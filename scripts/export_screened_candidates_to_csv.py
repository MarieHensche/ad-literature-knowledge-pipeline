#!/usr/bin/env python3
"""Export included screened candidates to the canonical paper CSV format."""
...
def main() -> None:
    ...
    screening_rows = read_csv(Path(args.screening))

    output_rows = export_included(candidates, screening_rows)
    write_csv(Path(args.output), output_rows)

    print(f"Screened rows: {len(screening_rows)}")
    print(f"Included rows exported: {len(output_rows)}")
